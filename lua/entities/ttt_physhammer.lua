...
--ENT.PunchMax = 12
--ENT.PunchRemaining = ENT.PunchMax
...
--         self:StopParticles()
         ...
         util.BlastDamage(self, self:GetOwner(), pos, 600, 125)
         ...
         if IsValid(self.PunchEntity) and IsValid(self.PunchEntity:GetPhysicsObject()) then
			...
			--local punchphys = self:GetPhysicsObject()
		end
            ...
            local base = 120
            local bonus = punchphys and punchphys:GetMass() * 2 or 0
            ...
            util.BlastDamage(self, self:GetOwner(), self:GetPos(), 400, 5)
            ...
            sound.Play(punchsound, self:GetPos(), 200, 100)
         end

      local delay = math.max(0.1, self.PunchRemaining / self.PunchMax) * 1
