...
function ENT:Think()
	...
	if self.timer < CurTime() then
		if !IsValid(self.Bastardgas) && !self.Spammed then
			...
			self.Bastardgas:SetKeyValue("startsize","800")
			self.Bastardgas:SetKeyValue("endsize","50")
			self.Bastardgas:SetKeyValue("endcolor","255 0 0")
			self.Bastardgas:SetKeyValue("startcolor","255 0 0")
			self.Bastardgas:SetKeyValue("opacity","3")
		...
		if (self.timer+100<CurTime()) then
			if IsValid(self.Bastardgas) then
				self.Bastardgas:Remove()
			end
		end
		if (self.timer+105<CurTime()) then
			self.Entity:Remove()
		end
