...
function GetSurvivalistFilter(alive_only)
   return GetPlayerFilter(function(p) return p:IsSurvivalist() and (not alive_only or p:IsTerror()) end)
end

function GetResurrectorFilter(alive_only)
   return GetPlayerFilter(function(p) return p:IsResurrector() and (not alive_only or p:IsTerror()) end)
end
...
function GetSerialkillerFilter(alive_only)
   return GetPlayerFilter(function(p) return p:IsSerialkiller() and (not alive_only or p:IsTerror()) end)
end

function GetInfectedFilter(alive_only)
   return GetPlayerFilter(function(p) return p:IsInfected() and (not alive_only or p:IsTerror()) end)
end
