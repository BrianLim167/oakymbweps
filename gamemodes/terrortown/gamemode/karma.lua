...
//   if not attacker:IsPlayer() or not victim:IsPlayer() then return end
   ...
   if attacker:IsPlayer() and attacker:GetRole() == ROLE_TRAITOR and victim:GetRole() ~= ROLE_TRAITOR then
      local reward = KARMA.GetHurtReward(hurt_amount)
      reward = KARMA.GiveReward(attacker, reward)
   ...
   elseif attacker:IsPlayer() and attacker:GetRole() == ROLE_TRAITOR and victim:GetRole() == ROLE_TRAITOR then
      if WasAvoidable(attacker, victim, dmginfo) then return end
   ...
   if attacker:IsPlayer() and table.HasValue( roletable, attacker:GetRole() ) and not table.HasValue( roletable, victim:GetRole() ) then
      ...
      reward = KARMA.GiveReward(attacker, reward)
         print(Format("%s (%f) killed %s (%f) and gets REWARDED %f", attacker:Nick(), attacker:GetLiveKarma(), victim:Nick(), victim:GetLiveKarma(), reward))
   elseif attacker:IsPlayer() and table.HasValue( roletable, attacker:GetRole() ) and table.HasValue( roletable, victim:GetRole() ) then
      if WasAvoidable(attacker, victim, dmginfo) then return end
   ...
   if attacker:IsPlayer() and attacker:GetRole() == ROLE_INFECTED and victim:GetRole() ~= ROLE_INFECTED then
      local reward = KARMA.GetHurtReward(hurt_amount)
      reward = KARMA.GiveReward(attacker, reward)
         print(Format("%s (%f) killed %s (%f) and gets REWARDED %f", attacker:Nick(), attacker:GetLiveKarma(), victim:Nick(), victim:GetLiveKarma(), reward))
   elseif attacker:IsPlayer() and attacker:GetRole() == ROLE_INFECTED and victim:GetRole() == ROLE_INFECTED then
      if WasAvoidable(attacker, victim, dmginfo) then return end
   ...
   if attacker:IsPlayer() and attacker:GetRole() == ROLE_SERIALKILLER and victim:GetRole() ~= ROLE_SERIALKILLER then
      local reward = KARMA.GetHurtReward(hurt_amount)
      reward = KARMA.GiveReward(attacker, reward)
         print(Format("%s (%f) killed %s (%f) and gets REWARDED %f", attacker:Nick(), attacker:GetLiveKarma(), victim:Nick(), victim:GetLiveKarma(), reward))
   elseif attacker:IsPlayer() and attacker:GetRole() == ROLE_SERIALKILLER and victim:GetRole() == ROLE_SERIALKILLER then
      if WasAvoidable(attacker, victim, dmginfo) then return end
