...
local function ReceiveRole()
   ...
   elseif client:IsResurrector() then MsgN("PHOENIX")
   elseif client:IsSerialkiller() then MsgN("SERIALKILLER")
   elseif client:IsInfected() then MsgN("INFECTED")
...
hook.Add( "PreDrawHalos", "AddHalos", function()
   ...
   if client and client:GetSerialkiller() then
	...
	for k, v in pairs( player.GetAll() ) do
		if v:GetRole() ~= ROLE_SERIALKILLER then
		if v:Alive() and v:GetRole() ~= ROLE_JESTER then
			table.insert( staff, v )
		elseif v:Alive() and v:GetRole() == ROLE_JESTER then
