...
local function GetTextForRole(role)
   ...
   elseif role == ROLE_SURVIVALIST then
      return GetPTranslation("info_popup_survivalist", {menukey = Key("+menu_context", "C")})
 
   elseif role == ROLE_JESTER then
      return GetPTranslation("info_popup_jester", {menukey = Key("+menu_context", "C")})
 
   elseif role == ROLE_RESURRECTOR then
      return GetPTranslation("info_popup_phoenix", {menukey = Key("+menu_context", "C")})
 
   elseif role == ROLE_SERIALKILLER then
      return GetPTranslation("info_popup_serialkiller", {menukey = Key("+menu_context", "C")})
 
   elseif role == ROLE_INFECTED then
      return GetPTranslation("info_popup_infected", {menukey = Key("+menu_context", "C")})
