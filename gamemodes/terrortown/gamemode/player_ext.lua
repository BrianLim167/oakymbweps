...
function plymeta:SetDefaultCredits()
   ...
   elseif self:GetSurvivalist() then
      self:SetCredits(math.ceil(GetConVarNumber("ttt_sur_credits_starting")))
   elseif self:GetSerialkiller() then
      self:SetCredits(math.ceil(GetConVarNumber("ttt_sk_credits_starting")))
   else
      self:SetCredits(0)
   end
end
