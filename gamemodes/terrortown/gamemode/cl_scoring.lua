...
CLSCORE.SurvivalistIDs = {}
CLSCORE.JesterIDs = {}
CLSCORE.ResurrectorIDs = {}
CLSCORE.SerialkillerIDs = {}
CLSCORE.InfectedIDs = {}
CLSCORE.Players = {}
CLSCORE.StartTime = 0
CLSCORE.Panel = nil
...
function CLSCORE:BuildScorePanel(dpanel)
   ...
   for id, s in pairs(scores) do
      if id != -1 then
         ...
         local role = was_traitor and T("traitor") or (s.was_detective and T("detective") or "") or was_survivalist and T("survivalist") or was_serialkiller and T("serialkiller")
...
local wintitle = {
   ...
   [WIN_JESTER] = {txt = "hilite_win_Jester", c = Color(238, 130, 238, 255)},
   [WIN_SERIALKILLER] = {txt = "hilite_win_serialkiller", c = Color(85,	26,	139, 255)},
   [WIN_INFECTED] = {txt = "hilite_win_infected", c = Color(29,	33,	13, 255)},
   [WIN_BEES] = {txt = "hilite_win_bees", c = Color(255,255,0, 255)},
...
function CLSCORE:BuildHilitePanel(dpanel)
   ...
   for k, afn in pairs(AWARDS) do
      local a = afn(self.Events, self.Scores, self.Players, self.TraitorIDs, self.DetectiveIDs, self.SurvivalistIDs, self.JesterIDs, self.ResurrectorIDs, self.SerialkillerIDs, self.InfectedIDs)
...
function CLSCORE:Init(events)
   ...
   local survivalist = nil
   local jester = nil
   local resurrector = nil
   local serialkiller = nil
   local infected = nil
   for k, e in pairs(events) do
      ...
      elseif e.id == EVENT_SELECTED then
		 ...
		 survivalist = e.survivalist_ids
		 jester = e.jester_ids
		 resurrector = e.resurrector_ids
		 serialkiller = e.serialkiller_ids
		 infected = e.infected_ids
      end
   ...
   scores = ScoreEventLog(events, scores, traitors, detectives, survivalist, jester, resurrector, serialkiller, infected)

   self.Players = nicks
   self.Scores = scores
   self.TraitorIDs = traitors
   self.DetectiveIDs = detectives
   self.SurvivalistIDs = survivalist
   self.JesterIDs = jester
   self.ResurrectorIDs = resurrector
   self.SerialkillerIDs = serialkiller
   self.InfectedIDs = infected
