...
local function IdentifyBody(ply, rag)
   ...
   if bodyfound:GetBool() and not CORPSE.GetFound(rag, false) then
	  ...
	  elseif role == ROLE_RESURRECTOR then
         roletext = "body_found_r"
	  elseif role == ROLE_SERIALKILLER then
         roletext = "body_found_sk"
	  elseif role == ROLE_INFECTED then
         roletext = "body_found_inf"
      else
...
function CORPSE.ShowSearch(ply, rag, covert, long_range)
   ...
   if ply:IsActiveTraitor() or ply:IsActiveDetective() or ply:IsActiveSurvivalist() or ply:IsActiveSerialkiller() then
