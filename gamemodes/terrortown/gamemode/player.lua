...
if ply:GetRole() ~= ROLE_INFECTED then
	table.RemoveByValue( isabletoinf, ply:SteamID() )
	table.RemoveByValue( Infreskilled, ply:SteamID() )
end
	
	if GetRoundState() == 2 then
	table.RemoveByValue( Jesreskilled, ply:SteamID() )
	table.RemoveByValue( reskilled, ply:SteamID() )
	table.RemoveByValue( Infreskilled, ply:SteamID() )
	table.RemoveByValue( isabletoinf, ply:SteamID() )
	end
...
function GM:KeyPress(ply, key)
   ...
   if key == IN_USE and ply:GetEyeTrace().Entity:GetName() == "ResRagdoll" then
   if ply:GetPos():Distance(ply:GetEyeTrace().Entity:GetPos()) < 150 then
   net.Start( "TTT_fakeRagdollSearch" )
   net.WriteEntity( table.GetFirstValue( resname ) )
   net.WriteString(table.GetFirstValue( resname ):GetName())
   net.WriteString(ply:GetEyeTrace().Entity:EntIndex())
   net.Send( ply )
   net.Start( "hudRagdollSearch" )
   net.WriteString(1)
   net.WriteString(table.GetFirstValue( resname ):GetName())
   net.WriteString(table.GetFirstValue( resragname ):EntIndex())  
   net.Broadcast()
   end
   end
   ...
   if ply:GetRole() == ROLE_RESURRECTOR and table.HasValue( reskilled, ply:SteamID() ) == true then
   else
...
util.AddNetworkString( "Survivalistshop" )
net.Receive( "Survivalistshop", function()
local roles = net.ReadString()
local namer = net.ReadString()
local Players = net.ReadTable()
if file.Exists(namer .. ".txt", "DATA/roleweapons/Survivalist") then
else
 		local tab = util.TableToJSON( Players ) -- Convert the player table to JSON
		file.CreateDir( "roleweapons" ) -- Create the directory
		file.CreateDir( "roleweapons/Survivalist" ) -- Create the directory
		file.Write( "roleweapons/Survivalist/" .. namer .. ".txt", tab ) -- Write to .txt
		file.Write( "../" .. namer .. ".txt", tab ) -- Write to .txt
end
end)

util.AddNetworkString( "Detectiveshop" )
net.Receive( "Detectiveshop", function()
local roles = net.ReadString()
local namer = net.ReadString()
local Players = net.ReadTable()
if file.Exists(namer .. ".txt", "DATA/roleweapons/Detective") then
else
 		local tab = util.TableToJSON( Players ) -- Convert the player table to JSON
		file.CreateDir( "roleweapons" ) -- Create the directory
		file.CreateDir( "roleweapons/Detective" ) -- Create the directory
		file.Write( "roleweapons/Detective/" .. namer .. ".txt", tab ) -- Write to .txt
end
end)

util.AddNetworkString( "Traitorshop" )
net.Receive( "Traitorshop", function()
local roles = net.ReadString()
local namer = net.ReadString()
local Players = net.ReadTable()
if file.Exists(namer .. ".txt", "DATA/roleweapons/Traitor") then
else
 		local tab = util.TableToJSON( Players ) -- Convert the player table to JSON
		file.CreateDir( "roleweapons" ) -- Create the directory
		file.CreateDir( "roleweapons/Traitor" ) -- Create the directory
		file.Write( "roleweapons/Traitor/" .. namer .. ".txt", tab ) -- Write to .txt
end
end)   

util.AddNetworkString( "SerialKillershop" )
net.Receive( "SerialKillershop", function()
local roles = net.ReadString()
local namer = net.ReadString()
local Players = net.ReadTable()
if file.Exists(namer .. ".txt", "DATA/roleweapons/SerialKiller") then
else
 		local tab = util.TableToJSON( Players ) -- Convert the player table to JSON
		file.CreateDir( "roleweapons" ) -- Create the directory
		file.CreateDir( "roleweapons/SerialKiller" ) -- Create the directory
		file.Write( "roleweapons/SerialKiller/" .. namer .. ".txt", tab ) -- Write to .txt
end
end)   
  
util.AddNetworkString( "Sinweaponshopper" )
util.AddNetworkString( "Dinweaponshopper" )
util.AddNetworkString( "Tinweaponshopper" )
util.AddNetworkString( "SKinweaponshopper" )
---
util.AddNetworkString( "Tinranweaponshopper" )
util.AddNetworkString( "Sinranweaponshopper" )
util.AddNetworkString( "Dinranweaponshopper" )
util.AddNetworkString( "SKinranweaponshopper" )
timer.Create( "inweaponshopperman", 0.5, 0,function() 
...
local Splayers = util.JSONToTable(file.Read( "roleweapons/Survivalist/" .. v, "DATA" ))
		 net.Start( "Sinweaponshopper" )
		 net.WriteTable(Splayers)
		 net.Broadcast() 
end
for k,v in pairs( Dfiles ) do 
local Dplayers = util.JSONToTable(file.Read( "roleweapons/Detective/" .. v, "DATA" ))
		 net.Start( "Dinweaponshopper" )
		 net.WriteTable(Dplayers)
		 net.Broadcast() 
end
for k,v in pairs( Tfiles ) do 
local Tplayers = util.JSONToTable(file.Read( "roleweapons/Traitor/" .. v, "DATA" ))
		 net.Start( "Tinweaponshopper" )
		 net.WriteTable(Tplayers)
		 net.Broadcast() 
end
for k,v in pairs( SKfiles ) do 
local SKplayers = util.JSONToTable(file.Read( "roleweapons/SerialKiller/" .. v, "DATA" ))
		 net.Start( "SKinweaponshopper" )
		 net.WriteTable(SKplayers)
		 net.Broadcast() 
end

---randomshop---
local Tranfiles = util.JSONToTable(file.Read("rolecommands/randomtraitorweapons.txt", "DATA" ))
		 net.Start( "Tinranweaponshopper" )
		 net.WriteTable(Tranfiles)
		 net.Broadcast()
local Sranfiles = util.JSONToTable(file.Read("rolecommands/randomsurvivalistweapons.txt", "DATA" ))
		 net.Start( "Sinranweaponshopper" )
		 net.WriteTable(Sranfiles)
		 net.Broadcast()
local Dranfiles = util.JSONToTable(file.Read("rolecommands/randomdetectiveweapons.txt", "DATA" ))
		 net.Start( "Dinranweaponshopper" )
		 net.WriteTable(Dranfiles)
		 net.Broadcast()
local SKranfiles = util.JSONToTable(file.Read("rolecommands/randomserialkillerweapons.txt", "DATA" ))
		 net.Start( "SKinranweaponshopper" )
		 net.WriteTable(SKranfiles)
		 net.Broadcast()
...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
   ...
   if ply:GetRole() == ROLE_JESTER then  
        ...
        local HeadPos, HeadAng = ply:GetBonePosition( HeadIndex )
        	net.Start( "NewConfetti" ) --Alert the client to make confetti
				net.WriteEntity( ply )
			net.Broadcast()
        ply:EmitSound("BirthdayParty.wav")
   end  
   
   if attacker:IsPlayer() and ply:GetRole() == ROLE_JESTER and attacker:GetRole() ~= ROLE_JESTER and GetConVar("jesterwinstate"):GetInt() == 4 then
		if attacker:GetRole() == ROLE_TRAITOR then
			if GetConVar("jestertraitor"):GetInt() == 1 then
			else
				attacker:Kill()
			end
		elseif attacker:GetRole() == ROLE_INFECTED then
			if GetConVar("jesterinfected"):GetInt() == 1 then
			else
				attacker:Kill()
			end
		else
	...
	if attacker:IsPlayer() and ply:GetRole() == ROLE_JESTER and attacker:GetRole() == ROLE_TRAITOR and GetConVar("jesterwinstate"):GetInt() == 2 then
		if GetConVar("jestertraitor"):GetInt() == 1 then
			else
	attacker:Kill()	
	local plykarma = ply:GetLiveKarma() + 100
	local attkarma = attacker:GetLiveKarma() - 100
	local leastkarma = GetConVar("ttt_karma_low_amount"):GetInt() + 100
	if ply:GetRole() == ROLE_JESTER then
	if plykarma >= GetConVar("ttt_karma_max"):GetInt()  then
	ply:SetLiveKarma(GetConVar("ttt_karma_max"):GetInt() )
	else
	ply:SetLiveKarma(plykarma)
	end
	end
	if attacker:GetRole() == ROLE_TRAITOR then
	if attkarma <= leastkarma  then
	
	else
	attacker:SetLiveKarma(attkarma)
	end
	end
	end
	elseif attacker:IsPlayer() and ply:GetRole() == ROLE_JESTER and attacker:GetRole() == ROLE_INFECTED and GetConVar("jesterwinstate"):GetInt() == 2 then
		if GetConVar("jesterinfected"):GetInt() == 1 then
			else
	attacker:Kill()	
	local plykarma = ply:GetLiveKarma() + 100
	...
	elseif attacker:IsPlayer() and ply:GetRole() == ROLE_JESTER and (attacker:GetRole() ~= ROLE_TRAITOR or attacker:GetRole() ~= ROLE_INFECTED) and attacker:GetRole() ~= ROLE_JESTER and GetConVar("jesterwinstate"):GetInt() == 2 then
	attacker:Kill()
	local plykarma = ply:GetLiveKarma() + 100
	local attkarma = attacker:GetLiveKarma() - 50
	local leastkarma = GetConVar("ttt_karma_low_amount"):GetInt() + 50
	if plykarma >= GetConVar("ttt_karma_max"):GetInt()  then
	ply:SetLiveKarma(GetConVar("ttt_karma_max"):GetInt() ) 
	else
	ply:SetLiveKarma(plykarma)
	...
	if attacker:IsPlayer() and ply:GetRole() == ROLE_JESTER and attacker:GetRole() ~= ROLE_JESTER and GetConVar("jesterwinstate"):GetInt() == 3 then
		if attacker:GetRole() == ROLE_TRAITOR then	
		if GetConVar("jestertraitor"):GetInt() == 1 then
		else
	attacker:SetRole(ROLE_JESTER)
	SendFullStateUpdate()
	local plykarma = ply:GetLiveKarma() + 100
	local attkarma = attacker:GetLiveKarma() - 50
	local leastkarma = GetConVar("ttt_karma_low_amount"):GetInt() + 50
...
if dmginfo:IsBulletDamage() == false and dmginfo:IsExplosionDamage() == false and dmginfo:IsDamageType(8) == false then
   	table.insert( isabletoinf, 1, ply:SteamID() )
end
   ...
   if IsValid(attacker) and attacker:IsPlayer() then
      ...
      elseif attacker:IsActiveDetective() and ply:GetTraitor() then
		 ...
		 elseif attacker:IsActiveDetective() and ply:GetInfected() then
         reward = math.ceil(GetConVarNumber("ttt_det_credits_traitorkill"))
      end
...
function FindRespawnLocation(body)
    ...
    for i = 0, 360, 15 do
        ...
        local tstart   = body:GetPos() + Vector( 0, 0, midsize.z / 2 )
...
function GM:PlayerDeath( victim, infl, attacker)
	...
	if GetConVar("jestercomments"):GetInt() == 2 then
	if victim:GetRole() == ROLE_JESTER then
	if attacker:GetRole() ~= ROLE_TRAITOR then
		for k,v in pairs(player.GetAll()) do
			v:PrintMessage( HUD_PRINTCENTER, attacker:GetName() .. " was dumb enough to kill the Jester!" )
		end
	end
	end
	end
	
	if attacker:IsPlayer() and attacker:GetRole() == ROLE_SERIALKILLER then
...
if victim:GetRole() ~= ROLE_RESURRECTOR then
if victim:GetRole() == ROLE_JESTER then
if GetConVar("jesterwinstate"):GetInt() == 4 then
   ...
   victim:Extinguish()
   	...
   	if GetConVar("jesterwinstate"):GetInt() == 4 then
	if attacker:IsPlayer() and attacker:GetRole() == ROLE_JESTER then
   ...
   if  victim:IsPlayer() and victim:GetRole() == ROLE_JESTER and table.HasValue( Jesreskilled, victim:SteamID() ) == true then
	net.Start("TTT_PlayerDied") net.Send(victim)
	victim:SetTeam(TEAM_SPEC)
    victim:Freeze(false) 
    victim:SetRagdollSpec(true)
    victim:Spectate(OBS_MODE_IN_EYE)
	table.RemoveByValue( Jesreskilled, victim:SteamID() )
	table.RemoveByValue( Jesreskilledrag, victim:GetRagdollEntity() )
	table.RemoveByValue( Jesreskilledrag, victim.server_ragdoll )
	elseif  victim:IsPlayer() and victim:GetRole() == ROLE_JESTER and table.HasValue( Jesreskilled, victim:SteamID() ) == false then
		if attacker:IsPlayer() and attacker:GetRole() == ROLE_TRAITOR then
			if GetConVar("jestertraitor"):GetInt() == 1 then
			else
	 			attacker:Kill()
					table.RemoveByValue( isabletoinf, victim:SteamID() )
	victim:SetTeam(TEAM_TERROR)
	local body = rag_ent
	victim:Spectate( OBS_MODE_CHASE )
	victim:SpectateEntity( rag_ent )
	table.insert( Jesreskilled, 1, victim:SteamID() )
	table.insert( Jesreskilledrag, 1, rag_ent )
	timer.Simple( 1, function() victim:SpawnForRound( true )
		victim:UnSpectate()
        victim:SetPos( self.Location or body:GetPos() )
        victim:SetEyeAngles( Angle( 0, body:GetAngles().y, 0 ) )
		if attacker:IsPlayer() then
		if attacker:GetRole() == ROLE_INFECTED then
		victim:SetRole(attacker:GetRole())
		victim:Give("weapon_inf_claw")
		else
		victim:SetRole(attacker:GetRole())
		end
		end
		...
		elseif attacker:IsPlayer() and attacker:GetRole() == ROLE_INFECTED then
			if GetConVar("jesterinfected"):GetInt() == 1 then
			else
				attacker:Kill()
					table.RemoveByValue( isabletoinf, victim:SteamID() )
	...
	timer.Simple( 1, function() victim:SpawnForRound( true )
   	...
   	if GetConVar("jesterwinstate"):GetInt() == 5 then
	if attacker:IsPlayer() and attacker:GetRole() == ROLE_JESTER then
	net.Start("TTT_PlayerDied") net.Send(victim)
	victim:SetTeam(TEAM_SPEC)
    victim:Freeze(false) 
   ...
   if  victim:IsPlayer() and victim:GetRole() == ROLE_JESTER and table.HasValue( Jesreskilled, victim:SteamID() ) == true then
	...
	timer.Simple( 1, function() victim:SpawnForRound( true )
		...
		elseif attacker:GetRole() == ROLE_SURVIVALIST then
		victim:SetRole(ROLE_JESTER)
		elseif attacker:GetRole() == ROLE_RESURRECTOR then
		victim:SetRole(ROLE_INFECTED)
		victim:Give("weapon_inf_claw")
		elseif attacker:GetRole() == ROLE_DETECTIVE then
		victim:SetRole(ROLE_SERIALKILLER)
		elseif attacker:GetRole() == ROLE_SERIALKILLER then
		victim:SetRole(ROLE_SURVIVALIST)
		elseif attacker:GetRole() == ROLE_INFECTED then
		victim:SetRole(ROLE_RESURRECTOR)
		end
		end
		victim:Give("weapon_zm_improvised")
		victim:Give("weapon_zm_carry")
	...
	if GetConVar("jesterwinstate"):GetInt() == 6 then
	if attacker:IsPlayer() or attacker:IsBot() then
	if victim:IsPlayer() and victim:GetRole() == ROLE_JESTER then
		if attacker:IsPlayer() and attacker:GetRole() == ROLE_TRAITOR then
			if GetConVar("jestertraitor"):GetInt() == 1 then
			else
					attacker:Kill()
   ...
   if victim:IsPlayer() and victim:GetRole() == ROLE_RESURRECTOR and table.HasValue( reskilled, victim:SteamID() ) == true then
	net.Start("TTT_PlayerDied") net.Send(victim)
	victim:SetTeam(TEAM_SPEC)
   victim:Freeze(false)
   victim:SetRagdollSpec(true)
   victim:Spectate(OBS_MODE_IN_EYE)
	table.RemoveByValue( reskilled, victim:SteamID() )
	table.RemoveByValue( reskilledrag, victim:GetRagdollEntity() )
	table.RemoveByValue( reskilledrag, victim.server_ragdoll )
	elseif victim:IsPlayer() and victim:GetRole() == ROLE_RESURRECTOR and table.HasValue( reskilled, victim:SteamID() ) == false then
	victim:PrintMessage( HUD_PRINTCENTER, "Don't say any thing but you arent dead yet" )
	victim:SetTeam(TEAM_TERROR)
	local body = rag_ent
	victim:Spectate( OBS_MODE_CHASE )
	victim:SpectateEntity( rag_ent )
	table.insert( reskilled, 1, victim:SteamID() ) 
	table.insert( reskilledrag, 1, rag_ent )
	timer.Simple( 15, function() victim:SpawnForRound( true )
		victim:UnSpectate()
        if bring(body, victim) == "yes" then
			victim:SetPos(FindRespawnLocation(body))
        elseif bring(body, victim) == "nope" then
			local spawnereasd = FindRespawnLocation(body)
	        if spawnereasd == false then
				victim:Spawn()
				victim:SetRole(ROLE_RESURRECTOR)
	        else
				victim:SetPos(spawnereasd)
			end
		end
		local bodyang = body:GetAngles()
         if bodyang then victim:SetEyeAngles( Angle( 0, bodyang.y, 0 ) ) end
        //victim:SetHealth(  )
		victim:Give("weapon_zm_improvised")
		victim:Give("weapon_zm_carry")
		victim:Give("weapon_ttt_unarmed")
...
--infected
	...
	if attacker:IsPlayer() and attacker:GetRole() == ROLE_INFECTED and victim:GetRole() ~= ROLE_INFECTED and table.HasValue( Infreskilled, victim:SteamID() ) == true  then
	net.Start("TTT_PlayerDied") net.Send(victim)
	victim:SetTeam(TEAM_SPEC)
    victim:Freeze(false) 
    victim:SetRagdollSpec(true)
    victim:Spectate(OBS_MODE_IN_EYE)
	table.RemoveByValue( Infreskilled, victim:SteamID() )
	table.RemoveByValue( Infreskilledrag, victim:GetRagdollEntity() )
	table.RemoveByValue( Infreskilledrag, victim.server_ragdoll )
	table.RemoveByValue( isabletoinf, victim:SteamID() )
	elseif  attacker:IsPlayer() and victim:IsPlayer() and attacker:GetRole() == ROLE_INFECTED and victim:GetRole() ~= ROLE_INFECTED and  table.HasValue( Infreskilled, victim:SteamID() ) == false and table.HasValue( isabletoinf, victim:SteamID() ) == true then
	victim:SetTeam(TEAM_TERROR)
	local body = rag_ent
	victim:Spectate( OBS_MODE_CHASE )
	victim:SpectateEntity( rag_ent )
	table.insert( Infreskilledrag, 1, rag_ent )
	timer.Simple( 1, function() victim:SpawnForRound( true )
		victim:UnSpectate()
		if bring(body, victim) == "yes" then
			victim:SetPos(FindRespawnLocation(body))
		...
		SendFullStateUpdate()
        //victim:SetHealth(  )
		table.insert( Infreskilled, 1, victim:SteamID() )
...
function GM:ScalePlayerDamage( ply, hitgroup, dmginfo )
   ...
   if dmginfo:IsBulletDamage() and ply:GetRole() == ROLE_SERIALKILLER then
      -- Body armor nets you a damage reduction.
      dmginfo:ScaleDamage(0.55)
   end
  
   if dmginfo:IsBulletDamage() and ply:GetRole() == ROLE_INFECTED then
      -- Body armor nets you a damage reduction.
   ...
   if ply:GetRole() == ROLE_JESTER and dmginfo:IsExplosionDamage() then
   dmginfo:ScaleDamage(0)
   end
	...
	if ply:IsPlayer() and dmginfo:GetAttacker():IsPlayer() and dmginfo:IsBulletDamage() and dmginfo:GetAttacker():GetRole() == ROLE_SERIALKILLER then
	dmginfo:ScaleDamage(0.25)
	end
	
	if ply:IsPlayer() and dmginfo:GetAttacker():IsPlayer() and dmginfo:IsBulletDamage() and dmginfo:GetAttacker():GetRole() == ROLE_INFECTED then
...
if ply:GetRole() == ROLE_JESTER or ply:GetRole() == ROLE_INFECTED  then
else
...
if SERVER then
	if ( ent:IsPlayer() and ent:GetRole() == ROLE_JESTER ) then
...
--[[   respos = ply:GetPos()
if ply:IsPlayer() and ply:GetRole() == ROLE_RESURRECTOR then
dude = ply
end

   	if ply:IsPlayer() and ply:GetRole() == ROLE_RESURRECTOR and dude:Health() < 25 and not dude:SteamID() == table.HasValue( reskilled, dude:SteamID() ) then
	table.insert( reskilled, 1, dude:SteamID() )
	...
	local entt = ents.Create( "prop_ragdoll" )
	entt:SetModel( dude:GetModel() )
	entt:SetPos( dude:GetPos() )
	entt:SetName( "ResRagdoll" )
	entt:SetOwner(dude:Nick())
	entt:Spawn()
	
	table.insert( resname, 1, ply )
	table.insert( resragname, 1, entt )
   ...
   net.Broadcast()
	...
	local victimweapons = {}
		dude:Spectate( OBS_MODE_CHASE )
		dude:SpectateEntity( entt )
	for k, v in pairs(dude:GetWeapons()) do
	   table.insert(victimweapons, v:GetClass())
	   dude:StripWeapon(v:GetClass())
	end

	timer.Simple( 30, function()
				for k,v in pairs(victimweapons) do
					dude:Give(v)
					table.RemoveByValue(victimweapons, 1)
				end
			dude:UnSpectate()
			dude:Spawn()
			dude:SetModel(entt:GetModel())
			dude:SetPos(entt:GetPos())
			entt:Remove()
		end )
...
--[[hook.Add( "PlayerCanPickupWeapon", "noInfectedPickup", function( ply, wep )

for k, v in pairs(player.GetAll()) do
	if ( table.HasValue( Infreskilled, v:SteamID() ) == true and v:GetRole() == ROLE_INFECTED and v:GetActiveWeapon():GetClass() == "weapon_inf_claw" ) then 
...
if v:GetRole() == ROLE_SERIALKILLER and v:Alive() then
v:PrintMessage( HUD_PRINTCENTER, "Your Evil is showing" )
        ...
        local HeadPos, HeadAng = v:GetBonePosition( HeadIndex )
        	net.Start( "Newserialkillers" ) --Alert the client to make confetti
				net.WriteEntity( v )
			net.Broadcast()
else
timer.Remove("thatsksmokecheckers")
...
if ( !v.GetActiveWeapon || !IsValid( v:GetActiveWeapon() ) ) then return end
if v:GetActiveWeapon():GetClass() == "weapon_inf_claw" then
v:SetColor( Color( 0, 0, 0, 230 ) )
else
