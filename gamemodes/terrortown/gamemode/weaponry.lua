...
local function OrderEquipment(ply, cmd, args)
   ...
   if not (ply:IsActiveTraitor() or ply:IsActiveDetective() or ply:IsActiveSurvivalist() or ply:IsActiveSerialkiller() ) then return end
...
table.insert(swep_table.CanBuy, ROLE_SURVIVALIST )
end
end
end
end
...
if swep_table.ClassName == v.name then
table.insert(swep_table.CanBuy, ROLE_SERIALKILLER )
end
end
end
end
