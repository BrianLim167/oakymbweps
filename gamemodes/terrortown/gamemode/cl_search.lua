...
local TypeToMat = {
   ...
   eq_radar="radar",
   eq_disg="disguise",
   role={[ROLE_TRAITOR]="traitor", [ROLE_DETECTIVE]="det", [ROLE_SURVIVALIST]="sur", [ROLE_JESTER]="jes", [ROLE_RESURRECTOR]="res", [ROLE_SERIALKILLER]="sk", [ROLE_INFECTED]="inf", [ROLE_INNOCENT]="inno"},
   c4="code",
   dmg=DmgToMat,
   wep=WeaponToIcon,
   head="head",
...
function PreprocSearch(raw)
   ...
   for t, d in pairs(raw) do
      ...
      elseif t == "role" then
		 ...
		 elseif d == ROLE_SURVIVALIST then
		    search[t].text = T("search_role_s")
		 elseif d == ROLE_JESTER then
		    search[t].text = T("search_role_j")
		 elseif d == ROLE_RESURRECTOR then
		    search[t].text = T("search_role_r")
		 elseif d == ROLE_SERIALKILLER then
		    search[t].text = T("search_role_sk")
		 elseif d == ROLE_INFECTED then
		    search[t].text = T("search_role_inf")
         else
