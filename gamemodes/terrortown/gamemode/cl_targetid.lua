...
local indicator_matinf = Material("vgui/ttt/icon_inf")
local indicator_col = Color(255, 255, 255, 130)
local indicator_coljes = Color(255, 255, 255, 130)
local indicator_colinf = Color(255, 255, 255, 130)
...
//The file that controlls the J sign on the Jesters head in game seen by the traitors
	  
   end
   
   if client:GetInfected() then
         dir = client:GetForward() * -1
   	  	  render.SetMaterial(indicator_matinf)
	  
      for i=1, #plys do
         ply = plys[i]
         if ply:GetRole() == ROLE_INFECTED and ply != client then
            pos = ply:GetPos()
            pos.z = pos.z + 74

            render.DrawQuadEasy(pos, dir, 8, 8, indicator_matinf, 180)
         end
      end
   end
...
function GM:HUDDrawTargetID()
   ...
   local target_survivalist = false
   local target_jester = false
   local target_resurrector = false
...
fakebodyres = false
   ...
   if cls == "prop_ragdoll" and fakebodyres == false and tonumber( table.GetFirstValue( fakebodyid ) ) == client:GetEyeTrace().Entity:EntIndex() then
	draw.DrawText( "Unidentified body", "TargetID", ScrW() * 0.5, ScrH() * 0.54, Color( 255, 255, 0, 255 ), TEXT_ALIGN_CENTER )
	draw.DrawText( "Corpse", "TargetID", ScrW() * 0.5, ScrH() * 0.565, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
	draw.DrawText( "Press E to search. ALT + E to search covertly.", "TargetID", ScrW() * 0.5, ScrH() * 0.585, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
   elseif cls == "prop_ragdoll" and fakebodyres == true and tonumber( table.GetFirstValue( fakebodyid ) ) == client:GetEyeTrace().Entity:EntIndex() then
    draw.DrawText( table.GetFirstValue( fakebodyname ), "TargetID", ScrW() * 0.5, ScrH() * 0.54, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
	draw.DrawText( "Corpse", "TargetID", ScrW() * 0.5, ScrH() * 0.565, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
	draw.DrawText( "Press E to search. ALT + E to search covertly.", "TargetID", ScrW() * 0.5, ScrH() * 0.585, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
   end
